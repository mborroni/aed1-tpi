...
TEST(histHabitacionalTEST, sinCasasEnRegion) {
    eph_h th;
    eph_i ti;
    leerEncuesta("eph_3c_2020_10", th, ti);

    vector<int> resultado_esperado = {};
    EXPECT_EQ(resultado_esperado, histHabitacional(th, ti, 1));
}

TEST(obtenerMaximoHabitaciones, maximoHabitaciones2) {
    eph_h th;
    eph_i ti;
    leerEncuesta("eph_3c_2020_10", th, ti);

    int resultado_esperado = 4;
    eph_h casasEnRegion = obtenerCasasEnRegion(th, 43);
    EXPECT_EQ(resultado_esperado, obtenerMaximoHabitaciones(casasEnRegion));
}